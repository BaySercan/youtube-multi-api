...
const WHISPER_CHUNK_DURATION_SECONDS = 600; // 10 minutes per chunk

// Helper function to get audio duration in seconds using ffprobe
    ...
    if (stats.size > WHISPER_MAX_FILE_SIZE) {
      ...
      try {
        ...
        logger.info("🎤 Whisper: Preparing chunked transcription", {
          ...
          estimatedChunks,
          chunkSize: `${WHISPER_CHUNK_DURATION_SECONDS / 60} minutes each`,
        });
...
// Helper function to transcribe audio using OpenAI Whisper API (supports chunked files)
async function transcribeWithWhisper(audioData, lang = "tr") {
  ...
  try {
    ...
    if (audioData.type === "chunked") {
      ...
      // Return null if all chunks failed
      if (successCount === 0) {
